...
def delete_vdb_entities(raw_node_name="Dumbledore", file_path=None):
    ...
    if not isinstance(data_list, list) or embedding_dim is None or not isinstance(matrix_b64, str):
        print("JSON 结构与预期不符，请检查 'data', 'embedding_dim', 'matrix' 字段。", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print(f"解码或 reshape matrix 失败：{e}", file=sys.stderr)
        sys.exit(1)
    ...
    if removed_count > 0:
        print(f"已删除 {removed_count} 个包含 '{raw_node_name}' 的实体。")
    else:
        print(f"未找到包含 '{raw_node_name}' 的实体，跳过删除。")
    ...
    try:
        ...
        print(f"重新编码 matrix 失败：{e}", file=sys.stderr)
        sys.exit(1)
